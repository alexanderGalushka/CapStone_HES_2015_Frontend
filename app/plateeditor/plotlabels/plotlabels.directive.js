        ...
        function uniqueMemebers(list,field) {
            ...
            for(var i = 0; i < list.length; i++) {
                for(var j = 0; j < list[i].length; j++) {
                    ...
                    if (list[i][j][field] != null && !containsMemeber(arr, list[i][j][field])) {
                        arr.push(list[i][j][field]);
                        //var tag = '<label style="background-color:hsl(20,50%,50%);">' + '&nbsp;&nbsp;&nbsp;' + '</label>';
                        //arrObj.push({'name':list[i][j][field],'description': list[i][j][field] + tag });
                        arrObj.push({'name':list[i][j][field] });
                    }
                }
            }
            return arrObj;
